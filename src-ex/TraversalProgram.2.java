...
//    		 TODO: i dont get this
    ...
    public void doBFS(boolean cef, boolean po){
    	...
    	while ( ( currentWhiteCityIndex = getNextWhiteCity() ) != -1 ){
    		...
    		while (!cq.isEmpty()){
    			city = cq.dequeue();
    			// go thorough all the routes of a city
    			for (int i = 0; i < city.getRoutes().length; i++){
    				if (po){
    					route = city.getSortedRoutes()[i];
    				} else {
    					route = city.getRoutes()[i];
    				...
    				if (cities[route.getDestination()].getCostTo() < 0){
    					// still white
    					// enqueuing city2 to cq and change to grey
    					city2 = cities[route.getDestination()];
    					whiteCities[route.getDestination()] = -1;
    					cq.enqueue(city2);
    					// setting cost
    					city2.setPathToVia(city, route);
    					...
    					// cost exists, updating to cheaper one
    					if ((city.getCostTo() + route.getCost()) < cities[route.getDestination()].getCostTo()){
    						city2 = cities[route.getDestination()];
    						// if it enqueued already?
    						if (!city2.getProcessingStatus()){
    							cq.enqueue(city2);
    						}
    						city2.setCostTo(city.getCostTo() + route.getCost());
    						city2.setPathToVia(city, route);
